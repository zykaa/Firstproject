		...
		stringer: {
			...
			dateToTimeStr14: function(a) {
				var b = a.getFullYear(),
					...
					f = this.time2FigFill(a.getMinutes()),
	...
	SewisePlayerSkin.ElementLayout = function(b) {
		...
		this.fullScreen = function(b) {
			if("not-support" == b) {
				...
				var f = parseInt(a("body").css("margin-top")),
					g = l - t + g - f + "px";
				c.css("left", j - d + b + "px");
				c.css("top", g)
			} else c.css("width", "100%"), c.css("height", "100%");
			b = parseInt(a(window).width()) - 360;
...
(function(a, b) {
	b(document).ready(function() {
		var a = SewisePlayer.IVodPlayer,
			...
			f = new SewisePlayerSkin.LogoBox(b),
			j = new SewisePlayerSkin.TopBar(b),
			l = new SewisePlayerSkin.ClarityWindow(b),
			d = new SewisePlayerSkin.ControlBar(b, i, j, l);
